...
public class AboutYouTeachers {
    ...
    private AboutYouTeachers.UserData getUserData(String email, String cnp) throws SQLException {
        ...
        String query = "SELECT facultate.profesor.nume, prenume, adresa, nrDeTelefon, IBAN, facultate.materie.nume as Departament, email, CNP FROM facultate.profesor join facultate.activitate_didactica ad on profesor.id = ad.idProfesor join facultate.materie on facultate.materie.id = ad.idMaterie WHERE (email = ? AND cnp = ?)";
